...
void USoundVisComponent::BP_StartCalculatingFrequencySpectrum(USoundWave* InSoundWaveRef, const float InSegmentLength)
{
	...
	if (!bSoundPlaying)
	{
		...
		//Wave->GetPrecacheState() == ESoundWavePrecacheState::Done
		ESoundWavePrecacheState Ans = InSoundWaveRef->GetPrecacheState();
		InSoundWaveRef->SetPrecacheState(ESoundWavePrecacheState::Done);
...
void USoundVisComponent::BP_ResumeCalculatingFrequencySpectrumAtSpecificTime(float Time)
{
	if (bSoundPlaying && bSoundPaused)
	{
		// Start the Sound at the Point where we left it
		AudioComponent->Play(Time);
